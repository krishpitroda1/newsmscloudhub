...
export default function ContactPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!success) {
      try {
        const relayRes = await fetch("https://formsubmit.co/ajax/info@smscloudhub.com", {
          ...
          body: JSON.stringify({
            _subject: `📥 New Contact Form Submission from ${form.name} (${form.company || "N/A"}) — SMSCloudHub`,
            Name: form.name,
            Company: form.company || "N/A",
            Email: form.email,
            Phone: form.phone || "N/A",
            Volume: form.volume || "Not specified",
            Channel: form.channel || "Not specified",
        ...
        if (relayRes.ok) {
          ...
          const backupRes = await fetch("https://formsubmit.co/ajax/krishpitroda09@gmail.com", {
            ...
            body: JSON.stringify({
              _subject: `📥 Contact Lead — ${form.name}`,
              Name: form.name,
              Email: form.email,
              Message: form.message,
            }),
    ...
    // Always show success confirmation screen to user
    setSubmitted(true);
    setLoading(false);
  };
