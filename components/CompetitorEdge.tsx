export default function CompetitorEdge() {
  const competitorBenchmarks = [
    ...
    },
    {
      ...
      focus: "GSMA-Aligned Wholesale Hubbing & CPaaS APIs",
      advantage:
  ...
  return (
    <section id="competitors" className="py-24 bg-[#0F1B2E]/60 border-t border-white/10 relative">
      <div className="max-w-[1240px] mx-auto px-6">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {competitorBenchmarks.map((item, idx) => (
            <div
              key={idx}
              className="bg-[#0F1B2E] border border-white/10 rounded-2xl p-6 relative hover:border-[#22D3EE]/50 transition-colors"
            >
              <div className="flex items-center justify-between gap-4 mb-3">
                <span className="font-mono text-xs text-[#22D3EE] uppercase tracking-wider font-bold">
                  {item.focus}
                </span>
                <span className="text-[11px] font-mono bg-white/5 border border-white/10 text-[#8DA0C0] px-2.5 py-0.5 rounded">
                  {item.competitor}
                </span>
              </div>
              <p className="text-sm text-[#F3F8FF] leading-relaxed">
                {item.advantage}
              </p>
            </div>
          ))}
